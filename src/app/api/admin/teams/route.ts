...
export async function POST(request: Request) {
  if (!(await isAdmin())) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
  try {
      const { app_number, team_name } = await request.json();
      if (!app_number || !team_name) return NextResponse.json({ error: 'Missing fields' }, { status: 400 });
      
      await addTeam({ app_number, team_name, topic: null, locked_until: null });
...
export async function PUT(request: Request) {
  if (!(await isAdmin())) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
  try {
      const { app_number, team_name, topic } = await request.json();
